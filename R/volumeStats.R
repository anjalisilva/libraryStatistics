...
#'         The user selected institute is provided for comparison.
#'          The median is provided for comparison.
#'   \item volumesAllData - Violin plots showing the distribution
#'         of volumes for all institutes in dataset uploaded by user for the
...
visVolumeData <- function(dataARL, institute, years = NA) {
  ...
  volumeInstType <- topVolumesInst %>%
    ...
    ggplot2::labs(y = "Volumes Held",
                  x = "Year",
                  fill = "Institute",
                  title = "Maximum Volumes Held by Institute Type") +
    ggplot2::theme_bw() +
    ggplot2::theme(text = element_text(size = 15, color = 'black'),
                   axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5, color = 'black', size = 15),
                   axis.text.y = element_text(color = 'black', size = 15)) +
  ...
  volumeAcademicPlot <- topVolumesAcademicInst %>%
    ...
    ggplot2::labs(y = "Volumes Held",
                  x = "Year",
                  fill = "Institute",
                  title = "Maximum Volumes Held by Academic Institute Type") +
    ggplot2::theme_bw() +
    ggplot2::theme(text = element_text(size = 15, color = 'black'),
                   axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5, color = 'black', size = 15),
                   axis.text.y = element_text(color = 'black', size = 15)) +
  ...
  volumeARLRankTop <- combinedRankData %>%
    ...
    ggplot2::labs(y = "Volumes Held",
                  ...
                  fill = "Institute") +
    ggplot2::theme_bw() +
    ggplot2::ggtitle(label = "Volumes Held by Institutes with Highest Investment ARL Rank",
  ...
  volumesAllData <- selectedData %>%
    ...
    ggplot2::labs(y = "Volumes Held",
                  x = "Year") +
    ggplot2::theme_bw() +
    ggplot2::ggtitle(label = "Distribution of Volumes Held in Dataset",
